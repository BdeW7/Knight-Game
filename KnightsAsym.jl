...
function getAttacked(space, lpn)
    ...
    a=space[1]
    b=space[2]
    out = [space, [a-1,b], [a-1,b+1],[a,b-1], [a,b+1],[a+1,b],[a+1,b+1]]
    i=length(out)
    while i>1
...
img = [colour_map[out_grid[i][j]] for i in eachindex(out_grid), j in eachindex(out_grid)]
